...
namespace WisdomScenic.Project.Infrastructure.RQCode
{
    public class RQCodeHelper
    {
        ...
        public static string GenerateSceneNo(string fristNo, string secnodNo, string thirdNo, int type)
        {
            string _result = string.Empty;
            for (int i = fristNo.Length; i <= 8; i++)
            {
                fristNo = "0" + fristNo;
            }
            for (int i = secnodNo.Length; i <= 8; i++)
            {
                secnodNo = "0" + secnodNo;
            } for (int i = thirdNo.Length; i <= 8; i++)
            {
                thirdNo = "0" + thirdNo;
            }
            _result = fristNo + secnodNo + thirdNo + type;
            return _result;
        }

        public static void GenerateSceneNo(string data, out string fristNo, out string secnodNo, out string thirdNo, out int type)
        {
            fristNo = data.Substring(0, 8);
            secnodNo = data.Substring(8, 16);
            thirdNo = data.Substring(16, 24);
            type = Convert.ToInt32(data.Substring(24));
