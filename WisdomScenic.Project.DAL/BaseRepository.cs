...
using WisdomScenic.Project.Domain;
...
namespace WisdomScenic.Project.DAL
{
    public class BaseRepository<T> : IDisposable where T : Entity, new()
    {
        ...
        private bool AllowCache(string ClassName)
        {
            bool allow = false;
            if (ClassName.Split(new Char[] { '_' }, StringSplitOptions.RemoveEmptyEntries)[0].Equals("T")) //只缓存Entity
            {
                if (!"T_LoginLog".Equals(ClassName) 
                    && !"T_Column".Equals(ClassName)
                    && !"T_Advertising".Equals(ClassName)
                    && !"T_Destination".Equals(ClassName)
                    && !"T_APILog".Equals(ClassName)
                    )
                {
                    allow = true;
                }
